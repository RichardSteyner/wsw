    ...
    if(ApexUtil.isOliTriggerInvoked){
        ...
        if(trigger.isInsert){
            ...
            for(OpportunityLineItem oli : trigger.new){
                if(oli.Quantity != trigger.oldMap.get(oli.Id).Quantity || oli.UnitPrice != trigger.oldMap.get(oli.Id).UnitPrice || oli.NS_Price_Code__c != trigger.oldMap.get(oli.Id).NS_Price_Code__c || oli.Description != trigger.oldMap.get(oli.Id).Description) oppIds.add(oli.OpportunityId);
            }
        }else{
            for(OpportunityLineItem oli : trigger.old) oppIds.add(oli.OpportunityId);
        }
