trigger AccountTrigger on Account (before insert, before update, after insert, after update) {
    ...
    if(ApexUtil.isAccountTriggerInvoked){
        if(trigger.isBefore){
            ...
            if(trigger.isInsert){
                for(Account account : trigger.new) if(account.NS_ID__c == null) createAccountIds.add(account.Id);
            }else{
                for(Account account : trigger.new) if(account.NS_ID__c == null) createAccountIds.add(account.Id);
            }
            if(!createAccountIds.isEmpty()) NetsuiteMethods.createCustomer(createAccountIds);
        }
    }
